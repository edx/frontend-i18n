...
#. The job to pull translations out of Transifex and commit them to your repo will call ``make pull_translations``.
   ...
   * If the languages you want to pull down differ from the default set listed in the ``frontend-i18n`` Makefile, you can modify your local Makefile as per the comment on the ``extract_translations`` target.
